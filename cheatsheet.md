...
- `r`                     ===> replace one character (`R` enter replace mode)
...
- `gt`                    ===> go to next tab
- `gu`                    ===> lower(`U` upper, `~` toggle) at cursor requires motion
- `gv`                    ===> reselct text
- `gw`                    ===> format
...
- `<C-g>j`                ===> go to line below (`<C-g>k` above)
...
- `:g!/{pattern}/d`       ===> deletes all lines without the pattern (`g!` is the same as using `v`)
...
- `sa{text_object}{char}` ===> add surrounding to text object
...
> [!CITE] Inspired by https://vim.rtorr.com/
